...
- 設定画面は **全画面ビュー切替**（TradingView/Linear 的）にして、その間は全 WebView を hide。
- ペイン追加は **DOM モーダル**（`uiStore.addPaneOpen`）。main ビュー（ペインストリップ等）はアンマウントせず張ったままにし、`overlay` を `modal` にして全 WebView を hide してからモーダルを重ねる。全画面ビュー切替だと復帰時に main が再マウントされ、placeholder の全解除→再登録→再計測が走って遅いため、頻度の高い開閉はモーダルにして復帰コストを WebView の `show()` のみに抑える。
- DnD 中はドラッグ対象（または全ペイン）の WebView を hide し、プレースホルダーカード（タイトル+アイコン）を表示。
- **幅リサイズは例外的にライブ表示**（`resizing` overlay は WebView を hide しない）。幅計算がカーソル絶対座標ベースのため、ポインタが WebView 上に出てイベントが途切れても状態は壊れず自己修復する（`useResizePane` の `buttons === 0` 検知）。速いドラッグでは bounds 同期ラグ分のカクつきを許容。完全解決が必要になったら透明キャプチャ WebView 案（リサイズ中だけ全面に透明な app オリジン WebView を重ねてポインタを捕捉）が次カード。
